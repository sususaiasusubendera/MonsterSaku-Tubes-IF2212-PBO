        ...
        try {
            ...
            for (String[] line : lines) {
                ...
                if (mvType.equals("STATUS")) {
                    ...
                    StatusMove move = new StatusMove(id, elType, accuracy, priority, ammunition, tOfMove, effect, healHP);
                    // tambahkan ke list moves
                    listMove.add(move);
                } else {
                    // bentuk object move jenis selain status move
                    Double damage = Double.parseDouble(line[8]);
                    Move move = new Move(id, mvType, elType, accuracy, priority, ammunition, tOfMove, damage);
                    // tambahkan ke list moves
                    listMove.add(move);
                }               
